...
export default function AddToShoppingList(props: { item: Item | undefined }) {
  ...
  const { isSmallScreen, isMediumScreen } = usePreparedMediaQuery();
  ...
  function onSuccess() {
    enqueueSnackbar(
      `${props?.item?.name ?? ""} is toegevoegd aan uw boodschappenlijst`,
      {
        variant: "info",
        action: (
          <IconButton
  ...
  return (
    <>
      <AddRecurringItemModal
        open={recurringItemModalOpen}
        handleClose={() => setRecurringItemModalOpen(false)}
        item={props.item}
      />
      {isSmallScreen || isMediumScreen ? (
        <IconButton
          ...
          className={"add-to-shopping-list-btn-sm"}
          sx={{
            justifySelf: "end",
            alignSelf: "start",
            marginTop: "2rem",
        ...
        <Button
          ...
          aria-haspopup="true"
          aria-expanded={open ? "true" : undefined}
          sx={{ justifySelf: "end", alignSelf: "start", marginTop: "2rem" }}
        >
          Toevoegen
        </Button>
      )}
