...
export default function HistoryChart(props: { trackedItems: TrackedItem[] }) {
  if (
    props.trackedItems.filter((e) => onePricePerDay(e.itemPrices).length > 1)
      .length === 0
  )
    return <></>;
    ...
    <>
      ...
      <VictoryChart
        ...
        maxDomain={{
          y:
            max(
              props.trackedItems.map((e) =>
                max(e.itemPrices.map((b) => b.price))
              )
            ) * 1.1,
        }}
        minDomain={{
          y:
            min(
              props.trackedItems.map((e) =>
                min(e.itemPrices.map((b) => b.price))
              )
            ) * 0.9,
        }}
      >
        <VictoryLegend
          ...
          data={props.trackedItems
            .filter((e) => e.itemPrices.length > 1)
            .map((ti) => {
              return {
                name:
                  ti.shop.name !==
        ...
        {props.trackedItems
          ...
          .map((trackedItem) => (
            <VictoryLine
              ...
              data={orderedPrices(onePricePerDay(trackedItem.itemPrices)).map(
                (e) => {
