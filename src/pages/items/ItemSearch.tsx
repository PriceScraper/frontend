...
export default function ItemSearch() {
    ...
    return (<>
        <MainContainer backTrackableTo={"/"} headerMsg={"Zoeken"} large={true}>
            ...
            {loading && <div style={{marginTop: 20}}><LoadingArea/></div>}
...
function SearchList({items}: { items: Item[] }) {
    const {isSmallScreen, isMediumScreen, isExtraBigScreen} = usePreparedMediaQuery()
    ...
    return (
        <ImageList variant="standard" cols={useCols} gap={isSmallScreen ? 10 : 15}
                   sx={{width: "100%", height: 350}}>
            {items.map((item) => (
                <ImageListItem sx={{cursor: "pointer"}} key={item.id} onClick={() => navigate(`/item/${item.id}`)}>
                    <img
                        src={`${item.image}`}
                        alt={item.name}
                        style={{maxWidth: useImgSize, maxHeight: useImgSize, margin: "1px auto"}}
                        loading="lazy"
                    />
                    <ImageListItemBar
