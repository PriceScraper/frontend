...
export default function SettingsPage() {
  ...
  useEffect(() => {
    axios
      .get<RecurringItem[]>("/shoppinglists/items/recurring")
      .then((res) => {
  ...
  function handleRemoveRecurringItem(id: number) {
    axios
      .delete(`/shoppinglists/items/remove/recurring/${id}`)
      .then(() => {
          ...
          <Box sx={{ paddingLeft: "2rem" }}>
            <FormGroup sx={{ marginTop: "0.7rem" }}>
              {availableShops.map((availableShop) => (
                <FormControlLabel
                  key={availableShop.shopId}
                  control={
                    <Checkbox
                      size={"small"}
                      checked={whiteListedShops
                        .map((ws) => ws.shopId)
                        .includes(availableShop.shopId)}
                      inputProps={{ "aria-label": "controlled" }}
                      onChange={(e) =>
                        whiteListedShops
                          .map((ws) => ws.shopId)
                          .includes(availableShop.shopId)
                          ? deleteWhitelistedShop(availableShop.shopId)
                          : addWhiteListedShop(availableShop)
                      }
                    />
                  }
                  label={
                    availableShop.shopName.substring(0, 1).toUpperCase() +
                    availableShop.shopName.substring(1)
                  }
                />
              ))}
            </FormGroup>
            ...
            <Box sx={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
              <Box sx={{ marginTop: "1rem", paddingRight: "1rem" }}>
                <Typography sx={{ fontWeight: "600" }}>
                  Periodieke producten
                </Typography>{" "}
                <Typography sx={{ color: "#343a40" }}>
                  Stel in welke producten u telkens in uw boodschappenlijst zou
                  ...
                  {recurringItems.map((recurringItem) => (
                    <FormControlLabel
                      ...
                      control={
                        <CloseIcon
                          onClick={() =>
                            handleRemoveRecurringItem(recurringItem.id)
                          }
